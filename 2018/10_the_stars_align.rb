...
lines = DATA.readlines(chomp: true).freeze
starmap = lines.map do |line|
  mo = /<(.*), (.*)>.*<(.*), (.*)>/.match(line)
  mo.captures.map(&:to_i).freeze
end.freeze
...
while true
  min_x = starmap.map {|s| s[0] + iter * s[2]}.min
  max_x = starmap.map {|s| s[0] + iter * s[2]}.max
  min_y = starmap.map {|s| s[1] + iter * s[3]}.min
  max_y = starmap.map {|s| s[1] + iter * s[3]}.max

  size = (max_x - min_x) + (max_y - min_y)
  if (box_size == -1) || (box_size > size)
    box_size = size
    iter += 1
  else
    iter -= 1
...
message_field = Array.new(max_y - min_y + 1) {Array.new(max_x - min_x + 1) {' '}}
starmap.each do |s|
  x = s[0] + iter*s[2] - min_x
  y = s[1] + iter*s[3] - min_y
  message_field[y][x] = '*'
end

puts 'Part 1.'
message_field.each do |line|
  puts line.join
end
